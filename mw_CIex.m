...
function [LCL, UCL, constructionSuccess,  previousSuccess, q0Current, ...
            ...
            = mw_CIex(X,h,alpha, glPoints, glLP, previousSuccess, q0Previous, ...
    ...
    h_low = temp(end) ;
    if h_low == lgh_grid(end)
        % h is beyond the values of nulltab. Set to the maximum value
        h_low = exp(h_low);
        h_high = h_low;
        boundaryFlag = 1;
    else
        % then h is on the precomputed grid
