% function used to compute the log density of Ys and Xsk=
function I = log_fyx_tce(xs,ys,q,xi, glPoints, glLP)


% nsim appears to be number of simulations, with different datasets
% supplied as rows
  nGQ = length(glPoints);
  [nsim,k] = size(xs) ;
  P4 = nan(nsim,nGQ) ;    
  xlow = (1000^(-xi)-1)/xi ;
      if xi>0
         if min(ys)>0 && max(-1/xi,xlow)>q
             ...
             if min(ys)>0
                [s1,w1] = glRescaler(glPoints,glLP, max(-1/xi,xlow),q) ; 
                s = repmat(s1',nsim,1);  w = repmat(w1',nsim,1) ;
              
             elseif max(ys)<0
                [s2,w2] = glRescaler(glPoints,glLP,gpcdf(max([q,-1/xi,xlow]),xi,3,max([q,-1/xi,xlow])),1) ;
 ...
 %          H0(find(isnan(s(:,1))),:) = -1e-10*ones(length(find(isnan(s(:,1)))),nGQ) ;             
      ...
      elseif xi < 0
          ...
          for j = 1:nsim
              if ys(j)<0
                  [s(:,j),w(:,j)] = glRescaler(glPoints,glLP,max(q,xlow),(ys(j)+xi*q)/xi/(1-ys(j))) ;                       
              elseif ys(j)>0&&ys(j)<1
                  [s(:,j),w(:,j)] = glRescaler(glPoints,glLP,max((ys(j)+xi*q)/xi/(1-ys(j)),xlow),q) ;
              else
                  if xlow > min([q,-1/xi,(ys(j)+xi*q)/xi/(1-ys(j))])
                     s(:,j) = ones(nGQ,1)*(min([q,-1/xi,(ys(j)+xi*q)/xi/(1-ys(j))])-1) ;
                     w(:,j) = ones(nGQ,1)/nGQ ;
                  else
                     [s(:,j),w(:,j)] = glRescaler(glPoints,glLP,xlow,min([q,-1/xi,(ys(j)+xi*q)/xi/(1-ys(j))])) ;    
                  end
